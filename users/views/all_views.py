...
def create_profile(request):
    ...
    try:
        ...
        serializer = UserRegistrationSerializer(
            user, data=mapped_data, partial=True,
            context={'email_verified': email_verified, 'updating_existing': True}
        )
    except User.DoesNotExist:
        if email_verified:
            serializer = UserRegistrationSerializer(data=mapped_data, context={'email_verified': True})
        else:
    ...
    if serializer.is_valid():
        try:
            ...
            import traceback
            return Response({'message': 'Error creating profile. Please try again.', 'error': str(e), 'debug_info': {'email_verified': email_verified,'email': email,'user_exists': User.objects.filter(email=email).exists()}}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    return Response({'message': 'Invalid data provided. Please check your information.','errors': serializer.errors,'debug_info': {'email_verified': email_verified,'email': email}}, status=status.HTTP_400_BAD_REQUEST)
...
def get_user_profile(request):
    ...
    try:
        user = User.objects.get(email=email)
        return Response({'success': True,'user_exists': True,'profile_completed': user.profile_completed,'data': {'id': user.id,'email': user.email,'firstName': user.first_name,'lastName': user.last_name,'phoneNumber': user.phone_number,'country': user.country,'city': user.city,'state': user.state,'linkedinUrl': user.linkedin_url,'background': user.background,'valueProposition': user.value_proposition,'areasOfExpertise': user.areas_of_expertise,'investmentExperience': user.investment_experience,'dealSizePreference': user.deal_size_preference,'industryFocus': user.industry_focus,'geographicFocus': user.geographic_focus,'currentRole': user.current_role,'company': user.company,'yearsExperience': user.years_experience,'education': user.education,'professionalExperience': user.professional_experience,'certifications': user.certifications,'achievements': user.achievements,'website': user.website,'bio': user.bio,'skills': user.skills,'languages': user.languages,'profileCompleted': user.profile_completed,'published': user.published,'createdAt': user.created_at.isoformat(),'updatedAt': user.updated_at.isoformat(),'resumeUrl': user.resume.url if user.resume else None,'buyerProfileUrl': user.buyer_profile.url if user.buyer_profile else None,'publicToken': str(user.public_token) if user.public_token else None,}}, status=status.HTTP_200_OK)
    except User.DoesNotExist:
        return Response({'success': True,'user_exists': False,'profile_completed': False,'data': {}}, status=status.HTTP_200_OK)
...
def health_check(request):
    return Response({'status': 'healthy','message': 'Searcher API is running','version': '1.0.0'}, status=status.HTTP_200_OK)
    ...
    try:
        ...
        try:
            ...
            if u and getattr(u, 'is_authenticated', False):
                user_info = {'email': getattr(u, 'email', None),'id': getattr(u, 'id', None),'published': getattr(u, 'published', None),'profile_completed': getattr(u, 'profile_completed', None),'public_token': str(getattr(u, 'public_token', None)) if getattr(u, 'public_token', None) else None,}
            else:
                user_info = None
        except Exception as e:
